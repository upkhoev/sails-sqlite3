...
module.exports = (function() {
  ...
  var adapter = {
     ...
     */
    registerConnection: function(connection, collection, cb) {
    ...
    find: function(connectionId, table, options, cb) {
      spawnConnection(connectionId, function __FIND__(client, cb) {
        ...
        Object.keys(connection.collections).forEach(function(collectionId) {
          schema[collection] = connection.collections[collectionId].schema;
        });
    ...
    create: function(connectionId, table, data, cb) {
      spawnConnection(connectionId, function __CREATE__(client, cb) {
        ...
        // Loop through all the attributes being inserted and check if a
        // sequence was used.
        Object.keys(collection.schema).forEach(function(schemaKey) {
          if (!utils.object.hasOwnProperty(collection.schema[schemaKey],
        ...
        client.run(query, attributes.values, function(err) {
          if (err) return cb(err);

          var query = 'SELECT * FROM ' + table + ' ORDER BY rowid DESC LIMIT 1';

          // Get the last inserted row
          client.get(query, function(err, row) {
            if (err) return cb(err);
            ...
            function setSequence(item, next) {
              ...
              var sequenceQuery = 'SELECT setval(' + sequenceName + ', ' +
                sequenceValue + ', true)';
