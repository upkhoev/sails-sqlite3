...
const JOURNAL_MODS = ['DELETE', 'TRUNCATE', 'PERSIST', 'MEMORY', 'WAL', 'OFF'];
...
const adapter = {
  ...
  registerDatastore: async function (datastoreConfig, physicalModelsReport, done) {
    ...
    try {
      ...
      if (datastoreConfig.journalMode) {
        if (JOURNAL_MODS.indexOf(datastoreConfig.journalMode) === -1) {
          return done(new Error('Consistency violation: Invalid journalMode: `' + datastoreConfig.journalMode + '` value.'));
        }
        writeClient.run('PRAGMA journal_mode = ?', [datastoreConfig.journalMode]);
      }
